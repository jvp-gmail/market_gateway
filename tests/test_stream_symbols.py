...
def test_put_stream_symbols_501_when_options_nonempty(client: TestClient, auth_headers: dict[str, str]) -> None:
...
def test_put_stream_symbols_400_when_both_empty(client: TestClient, auth_headers: dict[str, str]) -> None:
    client.app.state.stream_symbol_replace_queue = asyncio.Queue()
    r = client.put(
        "/events/stream/symbols",
