...
1. **Client lifecycle** — connect with existing OAuth/token path, heartbeat/admin handling per Schwab, backoff reconnect. *(Quote loop in `schwab/stream_equity_runner.py`: `LEVELONE_EQUITIES` for non-`/` symbols (includes Schwab **`$`** keys such as `$SPX`, `$TICK`), `LEVELONE_FUTURES` for symbols with a leading `/` e.g. `/ES`; gated by `ENABLE_SCHWAB_STREAMING` and non-empty `SCHWAB_STREAM_EQUITY_SYMBOLS` and/or `SCHWAB_STREAM_OPTIONS_SYMBOLS`.)* **schwab-py requires `add_level_one_*_handler` before `level_one_*_subs`** so the first DATA frames after SUBS are not dropped.
