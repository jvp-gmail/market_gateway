...
**Session resubscribe (same WebSocket):** `PUT /events/stream/symbols` with JSON `{"equities":["SPY"],"futures":["/ES"],"options":[]}` and `X-API-Key` replaces Schwab `LEVELONE_*` keys without reconnecting (at least one of equities or futures must be non-empty; **`options` must be `[]`** until option L1 is implemented). Returns **503** if streaming is not active.
